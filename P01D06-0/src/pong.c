...
int play_game(int first_move, int angle) {
    ...
    while (1) {
        ...
        if (gamer_num == 1) {
            printf("PLAYER LEFT TURN: ");
        } else {
            printf("PLAYER RIGHT TURN: ");
        }
        command = getchar();
        if (command != '\n') {
            while (getchar() != '\n') {}
        }
        // Move left or right racket
        if (gamer_num == 1) {
            if ((command == 'a' || command == 'A') && y_gamer_left > 1) {
                y_gamer_left -= 1;
            } else if ((command == 'z' || command == 'Z') && y_gamer_left < PLAYGROUND_HEIGHT - 2) {
                y_gamer_left += 1;
            }
        } else {
            if ((command == 'k' || command == 'K') && y_gamer_right > 1) {
                y_gamer_right -= 1;
            } else if ((command == 'm' || command == 'M') && y_gamer_right < PLAYGROUND_HEIGHT - 2) {
                y_gamer_right += 1;
            }
        }
        // Turn transition
